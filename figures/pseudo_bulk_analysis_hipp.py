...
save_df_hu_mu = pd.DataFrame(columns=["marker", "mean_primate", "mean_mouse", "ratio_mean", "pvalue"])
...
for i, mark in enumerate(list_feat_all):
    ...
    for pair in pairs_p:
        ...
        save_df_hu_mu.loc[mark] = [mark, mean1, mean2, np.log2(mean2/mean1), p]
...
corrected_pp
save_df_hu_mu["corrected_p"] = corrected_pp
save_df_hu_mu["qvalue"] =-np.log(corrected_pp)

lst_tmp = (save_df_hu_mu["ratio_mean"]>0).values.astype(int)
list_color = ["#F46197" if it else "#246B6B" for it in lst_tmp]
print(list_color)
dico_color = {mkk:ccl for mkk,ccl in zip(save_df_hu_mu.index.tolist(), list_color)}
print(dico_color)
...
if True:
    ...
    ax.hlines(y=xlim,xmin=-2.0,  xmax =2, color= 'k', linestyle='--', linewidth=0.8)
