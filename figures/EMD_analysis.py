...
def compute_pairwise_emd_specie(df, 
    ...
    if not os.path.exists(dir_path + "df_wd_species.csv"):
        df_hu_Ma = df[(df.Species.isin(list_species)) 
                & (df.aec.isin(list_clusters_primates))]
        species_pairs = list(itertools.combinations(list_species, 2))
        print(species_pairs)
        df_ks = pd.DataFrame(columns=["type","marker", "cluster1", "Species1", "cluster2", "Species2", "wassertein"])
        #print(df_hu_Ma)
        index=0
        cluster_method = "aec"
        list_cl = df_hu_Ma[cluster_method].unique()
        ...
        for sp1, sp2 in species_pairs:
            for cl,cl2 in list_of_pairs:
                    ...
                    df_cl = df_hu_Ma[(df_hu_Ma["aec"] == cl)]

                    dfm = df_cl.melt(id_vars=['Species'], value_vars=list_feat_all, var_name="marker")
                    #print(dfm)
                    gggg = dfm.groupby(["Species", "marker", "value"]).size().reset_index()
                    gggg.rename({0:"count"}, axis=1, inplace=True)
                    gggg["log_count"] = gggg["count"].map(np.log)

                    df_cl_hu = gggg[gggg.Species==sp1]
                    df_cl_Ma = gggg[gggg.Species==sp2]

                    df_cl2 = df_hu_Ma[(df_hu_Ma["aec"] == cl2)]

                    dfm2 = df_cl2.melt(id_vars=['Species'], value_vars=list_feat_all, var_name="marker")
                    gggg2 = dfm2.groupby(["Species", "marker", "value"]).size().reset_index()
                    gggg2.rename({0:"count"}, axis=1, inplace=True)
                    gggg2["log_count"] = gggg2["count"].map(np.log)

                    df_cl_hu2 = gggg2[gggg2.Species==sp1]
                    df_cl_Ma2 = gggg2[gggg2.Species==sp2]
                    ...
                    for it in list_feat_all:
                        ...
                        #print(X_dist)
                        Y = df_cl_Ma[df_cl_Ma.marker==it]["value"]
                        ...
                        #print(df_ks)
                        if cl != cl2:
                            Y2 = df_cl_Ma2[df_cl_Ma2.marker==it]["value"]
...
def pairwise_comp_per_subject(df, list_clusters_primates,
    ...
    df_hu_Ma = df[(df.Species.isin(list_species)) & (df.aec.isin(list_clusters_primates))]
    df_wd_subject = pd.DataFrame(columns=["type","marker", "cluster1", "Subject1", "cluster2", "Subject2",  "wassertein"])
    #print(df_hu_Ma)
    index=0
    list_sub = df_hu_Ma.Sample_num.unique()
    cluster_method = "aec"
    list_cl = df_hu_Ma[cluster_method].unique()
    list_of_pairs = [(list_cl[p1], list_cl[p2]) for p1 in range(len(list_cl)) for p2 in range(p1+1,len(list_cl))]
    for cl,cl2 in list_of_pairs:
            ...
            for sub1 in list_sub:
                for sub2 in list_sub:
                    if sub1!=sub2:
                        ...
                        df_cl_Ma = gggg[gggg.Sample_num==sub2]
                        ...
                        df_cl_Ma2 = gggg2[gggg2.Sample_num==sub2]
...
def plot_emd_comparison(df_ks, dir_path, key="subject"):
    palette = {"intra-cluster":"#ffcc00","inter-cluster":"#002255"}
    ...
    sns.set_style("white")
    box_pair = [((Ma,"intra-cluster"), (Ma,"inter-cluster")) for Ma in df_ks_m.marker.unique()]
    fig_args = {'x': "marker",
                'y': "wassertein",
                'hue':"type",
                'data': df_ks_s,
    ...
    configuration = {'test':'Mann-Whitney',
                     ...
                     'text_format':'star',}
    g = sns.stripplot(ax=ax,palette=palette, **fig_args,)
    ...
    fig.tight_layout()
    plt.savefig(dir_path + "wassertein_stripplot_" + key + ".svg")
    plt.close("all")
    ...
    g = sns.barplot(y="wassertein", x="type", data=df_ks_s, ax=ax,
            palette=palette,
            hue="type", order=['intra-cluster','inter-cluster'])
    annotator = Annotator(ax, box_pair, data=df_ks_s, x='type',
            y='wassertein',
            hue="type", order=['intra-cluster','inter-cluster'])
    ...
    fig.tight_layout()
    plt.savefig(dir_path + "wassertein_between_distributions_"
            + key + ".svg")
    plt.close("all")
...
def plot_emd_subject(df_wd_subject, dir_path, key="subject"):
    ...
    sns.set(font_scale=2.5)
    sns.set_style("white")
    box_pair = [((Ma,"intra-cluster"), (Ma,"inter-cluster")) for Ma in df_wd_subject_m.marker.unique()]
    fig_args = {'x': "marker",
                'y': "wassertein",
                'hue':"type",
    ...
    plt.savefig(dir_path + 
            "wassertein_stripplot_sunject_emd_comparison" + ".svg")
    plt.show()
    plt.close("all")
...
def main():
    region = "ctx"
    for region in ["hipp", "ctx", "str"]:
        print(region)
        if region == "ctx":
            params = params_visu.params_ctx
            list_species = ["Hu","Ma", "Mu"]
        elif region == "str":
